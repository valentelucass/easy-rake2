...
try {
    ...
    $sql = "UPDATE caixas SET deleted_at = NOW() WHERE id = ? AND deleted_at IS NULL";
    $stmt = executePreparedQuery($conn, $sql, "i", [$input["id"]]);
    if ($stmt === false) {
        sendResponse(false, "Erro ao excluir caixa", 500);
        exit;
    }
    ...
    $stmt->close();
    sendResponse(true, "Caixa excluída com sucesso", 200);
} catch (Exception $e) {
    error_log("Erro ao excluir caixa: " . $e->getMessage());
