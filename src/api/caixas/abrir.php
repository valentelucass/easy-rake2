...
try {
    ...
    $sql = "UPDATE caixas SET status = 'aberto', data_abertura = NOW() WHERE id = ? AND deleted_at IS NULL";
    $stmt = executePreparedQuery($conn, $sql, "i", [$input["id"]]);
    if ($stmt === false) {
        sendResponse(false, "Erro ao abrir caixa", 500);
        exit;
    }
    ...
    $stmt->close();
    sendResponse(true, "Caixa aberto com sucesso", 200);
} catch (Exception $e) {
