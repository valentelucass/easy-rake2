...
try {
    ...
    $sql = "UPDATE unidades SET deleted_at = NOW() WHERE id = ? AND deleted_at IS NULL";
    $stmt = executePreparedQuery($conn, $sql, "i", [$input["id"]]);
    if ($stmt === false) {
        sendResponse(false, "Erro ao excluir unidade", 500);
        exit;
    }
    ...
    $stmt->close();
    sendResponse(true, "Unidade excluída com sucesso", 200);
} catch (Exception $e) {
